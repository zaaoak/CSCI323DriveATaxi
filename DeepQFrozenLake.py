...
# PossibleStates = list(product(x_range, y_range))
...
class TaxiDQL():
    ...
    def train(self, episodes, render = False):
        ...
        PolicyNet = DQN(in_states=num_states, h1_nodes=num_states, out_actions=num_actions)
        TargetNet = DQN(in_states=num_states, h1_nodes=num_states, out_actions=num_actions)
            ...
            while not terminated and not truncated:
                ...
                if rng.random() <epsilon:
                    ...
                    with torch.no_grad():#no grad just prevents pytorch from calculating backpropagation values, as we just want the prediction
                        action = int(PolicyNet(state_to_dqn_input(CurrentState, num_states)).argmax().item())
        ...
        for state, action, new_state, reward, terminated in mini_batch:
            ...
            # When in a terminated state, target q value should be set to the reward.(reward + discount * futureQ ) future Q is zero for terminal state
                ...
                with torch.no_grad():
                    ...
                    target_value = reward + self.discount_factor_g * target_dqn(state_to_dqn_input(new_state, num_states)).max()
                    #print(target_value)
    ...
    def print_dqn(self, dqn,printall=False):
        ...
        for s in range(num_states):
            ...
            best_action = action_names[dqn(state_to_dqn_input(s, num_states)).argmax()]
